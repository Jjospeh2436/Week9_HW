...
//  setup or hooks
    ...
    // takes in 2 arguments, 1st is function run, 2nd is variable we are monitoring
    useEffect(() => {
        ...
        onValue(cartRef, (snapshot) => {
            ...
            // whats coming back from the database is esentially a dictionary/object
    ...
    const deleteItem = async (cartItem: ShopProps ) => {
        ...
        remove(itemRef)
        .then(() => {
            setMessage('Successfully Deleted Your Cart')
            setMessageType('success')
            setOpen(true)
        })
        .then(() => {
    ...
    const checkOut = async () => {
        ...
        if (response.status === 200){ //200 is a good status code
            remove(cartRef) // this is emptying the cart
            .then(() => {
