...
const AddToCart = (cart: CartProps ) => {
    ...
    const onSubmit: SubmitHandler<SubmitProps> = async(data: SubmitProps, event) => {
        ...
        // takes in two arguments, 1st is where we are pushing, 2nd is what we are psuhing
        push(cartRef, myCart)
        .then((_newCartRef) => {
    ...
    return (
        <Box>
            <form onSubmit = {handleSubmit(onSubmit)}>
                <Box>
                    <label htmlFor='quantity'>How much of {myCart.name}</label>
                    ...
                    <Grid item key={index} xs={12} md={6} lg={4}>
                        <Card sx={ shopStyles.card}>
                            ...
                            <CardContent>
                                ...
                                <Button size='medium' variant='outlined' sx={shopStyles.button} onClick = {()=>{ setCurrentShop(shop) ; setCartOpen(true)}}>                            
